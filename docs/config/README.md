...
Documentation for each JSON file in `config/<config-set>` (the default configuration files are in `config/default`). Adding game content generally requires only editing these files — see each doc for when a code change is also required.
