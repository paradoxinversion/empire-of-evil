...
export const getActiveEffectsOnPerson = (state: GameState, personId: string): EffectInstance[] => {
...
The following config files do not yet exist and need to be created:

| File | Purpose |
|---|---|
| `config/buildings.json` | Building type definitions (type, capacity, resource output, upkeep) |
| `config/plots.json` | All plot definitions (requirements, stages, effects) |
| `config/citizenActions.json` | The ~18 daily citizen action types and their conditions |
| `config/pets.json` | Pet types and special actions |
| `config/evilTiers.json` | EVIL threshold breakpoints and world response descriptors |
| `config/cpuBehaviors.json` | CPU org behavior profiles and action weights |
