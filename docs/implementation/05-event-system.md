...
export const resolveEvent = (
  ...
  state.eventLog.push({ ...event, requiresResolution: false } as GameEventRecord);
};
```
...
The content of these messages is templated: the engine fills in entity names and outcomes from the log. Template files live alongside config (e.g., `config/narrative/interceptedComms.json`) and follow the same Zod schema validation pattern.

## EVIL Tier Transition Events

When `perceived EVIL` crosses a tier boundary (see `config/evilTiers.json`), an interrupt event fires before the next day runs:
