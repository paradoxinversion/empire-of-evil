...
5. **Building Generation** — Zones tiles are populated with buildings based on their `generation_wealth` value and terrain type.
...
2. A starting tile in an existing zone is selected — either randomly or from a filtered list based on player preference (terrain type, nation, etc.).
3. Citizens in the starting tile become empire subjects. Their loyalty begins at a moderate level (60).
4. A portion (10) of the starting zone's citizens are recruited as the empire's first agents.
