import {
  LayoutDashboard, Globe, Users, DollarSign,
  FlaskConical, Target, Activity, ScrollText,
} from 'lucide-react';
  ...
  return (
    <nav className="bg-bg-surface border-r border-border-subtle w-[200px] flex-shrink-0 flex flex-col py-2">
      ...
      <div className="mt-auto px-3.5 pb-3 pt-3 border-t border-border-subtle">
        {/* EVIL score widget */}
        <div className="mb-3">
          <div className="font-mono text-[9px] tracking-[0.12em] text-text-muted mb-1">EVIL RATING</div>
          <div className="font-mono text-[22px] leading-none mb-0.5" style={{ color: tier.color }}>
            {evilScore}
          </div>
          <div className="font-mono text-[9px] tracking-[0.12em] mb-1.5" style={{ color: tier.color }}>
        ...
        <div>
          <div className="font-mono text-[9px] tracking-[0.12em] text-text-muted mb-1">INFRASTRUCTURE</div>
          <div className={`font-mono text-[11px] mb-1 ${infraPercent >= 100 ? 'text-negative' : infraPercent >= 80 ? 'text-warning' : 'text-positive'}`}>
            {infraZones} / {INFRA_CAP} ZONES
          </div>
          <ProgressBar
            value={infraPercent}
            color={infraPercent >= 100 ? 'negative' : infraPercent >= 80 ? 'warning' : 'positive'}
            height={2}
          />
        </div>
      </div>
