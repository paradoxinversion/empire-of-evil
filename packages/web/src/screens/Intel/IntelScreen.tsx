...
function WorldStatePanel({ gameState }: { gameState: GameState }) {
  const { nations, zones, governingOrganizations, eventLog, empire } = gameState;
  ...
  const orgRows: Row[] = Object.values(nations)
    ...
    .map(n => ({
      ...
      zones: String(Object.values(zones).filter(z => z.nationId === n.id).length),
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <Panel title="RECENT INTEL FEED">
        {recentFeed.length === 0 ? (
          <div className="text-text-muted text-[11px] py-2">No intelligence gathered yet.</div>
        ) : (
...
function getZoneTerrain(zone: { tileIds: string[] }, tiles: GameState['tiles']): string {
...
function ZoneDetailPanel({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'citizens' | 'activity'>('overview');
  ...
  if (!zone) {
    return (
      <Panel>
        <div className="text-text-muted text-[11px]">Zone not found.</div>
      </Panel>
    );
  }
  ...
  const citizenRows: Row[] = zonePersons.map(p => ({
    ...
    status: p.agentStatus && zone.intelLevel > INTEL_NAMES_THRESHOLD
    ...
    <div className="flex flex-col gap-3">
      <Panel>
        <div className="flex items-start justify-between mb-1">
          <div className="font-mono text-sm text-text-primary">{zone.name}</div>
          <button
            type="button"
            onClick={onClose}
      ...
      <Panel>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3 text-[11px]">
              <div>
                <div className="text-text-muted mb-0.5">POPULATION</div>
                <div className="font-mono text-text-primary">{zone.population.toLocaleString()}</div>
              </div>
              <div>
                <div className="text-text-muted mb-0.5">ECONOMIC OUTPUT</div>
                <div className="font-mono text-text-primary">${zone.economicOutput.toLocaleString()}</div>
              </div>
              <div>
                <div className="text-text-muted mb-0.5">WEALTH INDEX</div>
                <div className="font-mono text-text-primary">{Math.round(zone.generationWealth)}</div>
              </div>
              <div>
                <div className="text-text-muted mb-0.5">TILE COUNT</div>
                <div className="font-mono text-text-primary">{zone.tileIds.length}</div>
              </div>
            </div>

            <div>
              <div className="flex justify-between text-[11px] mb-1">
                <span className="text-text-muted">INTEL LEVEL</span>
                <span className="font-mono text-text-primary">{zone.intelLevel}%</span>
              </div>
              <ProgressBar value={zone.intelLevel} color="info" height={2} />
            </div>

            <div>
              <div className="text-text-muted text-[10px] tracking-[0.08em] mb-1.5">KNOWN BUILDINGS</div>
              {knownBuildings.length === 0 ? (
                <div className="text-text-muted text-[11px]">No known buildings.</div>
              ) : (
                <ul className="space-y-1">
                  {knownBuildings.map(b => (
                    <li key={b.id} className="text-[11px] text-text-secondary font-mono">
                      · {b.name}
                    </li>
                  ))}
                </ul>
        ...
        {activeTab === 'activity' && (
          activityFeed.length === 0 ? (
            <div className="text-text-muted text-[11px] py-2">No recorded activity.</div>
          ) : (
  ...
  return (
    <div>
      ...
      <div className="flex gap-3" style={{ minHeight: '640px' }}>
        {/* Left — map */}
        <div style={{ flex: '0 0 60%' }} className="flex flex-col gap-2">
          {/* Layer toggle */}
