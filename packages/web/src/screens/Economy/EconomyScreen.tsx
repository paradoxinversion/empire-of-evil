...
export function EconomyScreen() {
  ...
  const buildingIncomeByZone = getBuildingIncomeByZone(gameState, BUNDLED_CONFIG);
  ...
  const empireZones = Object.values(zones).filter(z => z.governingOrganizationId === empire.id);
  ...
  for (const b of Object.values(buildings)) {
    buildingCountByType[b.typeId] = (buildingCountByType[b.typeId] ?? 0) + 1;
    const def = BUNDLED_CONFIG.buildings.find(d => d.id === b.typeId);
    const income = def?.resourceOutput?.money ?? 0;
    buildingOutputByType[b.typeId] = (buildingOutputByType[b.typeId] ?? 0) + income;
  }
  ...
  const buildingIncomeRows: Row[] = Object.entries(buildingOutputByType)
    ...
    .map(([typeId, output]) => {
      ...
      return {
        ...
        output: (
          <span className={`font-mono text-[11px] ${output > 0 ? 'text-positive' : 'text-text-muted'}`}>
            {formatMoney(output)}
          </span>
        ),
      };
  ...
  const expenseRows: Row[] = [
    {
      ...
      amount: <span className="font-mono text-[11px] text-negative">{formatMoney(-agentSalaries)}</span>,
    },
    {
      _key: 'upkeep',
      category: 'Building Upkeep',
      amount: <span className="font-mono text-[11px] text-negative">{formatMoney(-buildingUpkeep)}</span>,
    },
    {
      _key: 'plots',
      category: 'Active Plots',
      amount: <span className="font-mono text-[11px] text-text-muted">$0</span>,
    },
    {
      _key: 'activities',
  ...
  const zoneEconomicsRows: Row[] = empireZones
    .map(z => {
      ...
      return {
        ...
        bldgInc: <span className="font-mono text-[11px]">{formatMoney(bldgInc)}</span>,
        taxInc:  <span className="font-mono text-[11px]">{formatMoney(taxInc)}</span>,
        upkeep:  <span className="font-mono text-[11px] text-negative">{formatMoney(-upkeep)}</span>,
        net:     (
          <span className={`font-mono text-[11px] ${net >= 0 ? 'text-positive' : 'text-negative'}`}>
            {formatMoney(net)}
          </span>
        ),
      };
  ...
  const agentCount = Object.values(persons).filter(p => p.agentStatus && !p.dead).length;
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <Panel title="BUILDING INCOME">
          <div className="font-mono text-base text-positive mb-2">{formatMoney(buildingIncome)}</div>
          <DataTable
        ...
        <Panel title="CITIZEN TAXES">
          <div className="font-mono text-base text-positive mb-2">{formatMoney(taxIncome)}</div>
          {empireZones.length === 0 ? (
            <div className="text-text-muted text-[11px]">No empire zones.</div>
          ) : (
            <DataTable
              columns={[
                { key: 'zone',    label: 'ZONE' },
        ...
        <Panel title="ACTIVITY INCOME">
          <div className="font-mono text-base text-text-muted mb-2">$0</div>
      ...
      <div className="mb-4">
        <Panel title="ZONE ECONOMICS">
          {zoneEconomicsRows.length === 0 ? (
            <div className="text-text-muted text-[11px]">No empire-controlled zones.</div>
          ) : (
            <DataTable
