...
export function InnerCircleTab({ onSelectPerson }: InnerCircleTabProps) {
  ...
  const removeInnerCircleMember = useGameStore(s => s.removeInnerCircleMember);
  ...
  const eligibleAgents = Object.values(persons).filter(
    person => !person.dead && person.agentStatus !== undefined && !empire.innerCircleIds.includes(person.id),
  );
      ...
      <div className="space-y-2">
        <Panel>
          <div className="flex items-center justify-between gap-2">
            ...
            <ActionButton onClick={() => setIsPickerOpen(true)}>+ ADD MEMBER</ActionButton>
          </div>
        </Panel>
        <AgentPicker
          isOpen={isPickerOpen}
          ...
          return (
            <Panel key={person.id}>
              <div className="flex justify-end">
                <button
                  type="button"
                  onClick={() => removeInnerCircleMember(person.id)}
