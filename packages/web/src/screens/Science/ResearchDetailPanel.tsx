...
function getUnlockNames(ids: string[], type: 'research' | 'plot' | 'activity'): string[] {
...
function AgentRow({
    ...
    return (
        <div className="flex items-center justify-between py-1 border-b border-bg-elevated last:border-0">
            <span className="font-mono text-[11px] text-text-primary">{person.name}</span>
            <ActionButton
...
export function ResearchDetailPanel({ ep, availableScientists, allProjects }: ResearchDetailPanelProps) {
    ...
    const removeAgentFromResearch = useGameStore(s => s.removeAgentFromResearch);

    const { definition: def, status, activeRecord, progressPct, daysRemaining, assignedAgents } = ep;

    const prereqMap = new Map(allProjects.map(p => [p.definition.id, p]));

    const unlockResearchNames = getUnlockNames(def.unlocks.researchIds, 'research');

    return (
        <Panel>
            {/* Header */}
            <div className="mb-3">
                <div className="font-mono text-[13px] text-text-primary mb-1">{def.name}</div>
                <div className="flex gap-1.5">
                    <Tag variant="research">{formatBranch(def.branch)}</Tag>
                    <Tag variant={STATUS_VARIANT[status]}>{status.toUpperCase()}</Tag>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1 mb-3 font-mono text-[10px]">
                <div>
                    ...
                    <span className="text-text-primary">${def.cost.toLocaleString()}</span>
                </div>
                <div>
                    <span className="text-text-muted">DURATION </span>
                    <span className="text-text-primary">{def.completionDays}d</span>
                </div>
                <div>
                    <span className="text-text-muted">SCI REQ </span>
                    <span className="text-text-primary">{def.scienceRequired}</span>
                </div>
            </div>
            ...
            {def.skillDrivers.length > 0 && (
                <div className="mb-3">
                    <div className="font-mono text-[9px] tracking-[0.1em] text-text-muted mb-1">SKILL DRIVERS</div>
                    <div className="flex flex-wrap gap-1">
                        {def.skillDrivers.map(skill => (
                            <Tag key={skill} variant="neutral">{skill.toUpperCase()}</Tag>
                        ))}
                    </div>
                </div>
            )}
            ...
            {def.prerequisites.length > 0 && (
                <div className="mb-3">
                    <div className="font-mono text-[9px] tracking-[0.1em] text-text-muted mb-1">PREREQUISITES</div>
                        ...
                        {def.prerequisites.map(id => {
                            ...
                            return (
                                <div key={id} className="flex items-center gap-1.5 font-mono text-[10px]">
                                    <span className={done ? 'text-positive' : 'text-text-muted'}>
                                        {done ? '✓' : '○'}
                                    </span>
            ...
            {(def.unlocks.researchIds.length > 0 ||
              ...
              def.unlocks.effectIds.length > 0) && (
                <div className="mb-3">
                    <div className="font-mono text-[9px] tracking-[0.1em] text-text-muted mb-1">UNLOCKS</div>
                    <div className="flex flex-col gap-0.5 font-mono text-[10px] text-text-secondary">
                        {unlockResearchNames.map(name => (
                            <div key={name}>
                                <Tag variant="research">RESEARCH</Tag>{' '}{name}
                            </div>
                        ))}
            ...
            {status === 'active' && activeRecord && (
                <div className="mb-3">
                    <div className="font-mono text-[9px] tracking-[0.1em] text-text-muted mb-1">PROGRESS</div>
                    <div className="mb-1">
                        <ProgressBar value={progressPct} color="info" height={4} />
                    </div>
                    ...
                    <div className="font-mono text-[10px] text-text-muted mt-0.5">
                        Score: {Math.round(activeRecord.accumulatedScore)} / {def.scienceRequired}
                    </div>
                </div>
            )}
            ...
            {status === 'active' && activeRecord && (
                <div className="mb-3">
                    <div className="font-mono text-[9px] tracking-[0.1em] text-text-muted mb-1">
                        ASSIGNED SCIENTISTS ({assignedAgents.length})
                    </div>
                    {assignedAgents.length === 0 ? (
                        <div className="text-[10px] text-text-muted">None assigned.</div>
                    ) : (
                        assignedAgents.map(person => (
                            <AgentRow
                                ...
                                action="remove"
                                actionLabel="REMOVE"
                                onAction={() => removeAgentFromResearch(activeRecord.id, person.id)}
                            />
                        ))
                    )}
                </div>
            ...
            {status === 'active' && activeRecord && availableScientists.length > 0 && (
                <div className="mb-3">
                    <div className="font-mono text-[9px] tracking-[0.1em] text-text-muted mb-1">
                        AVAILABLE SCIENTISTS
                    </div>
                    {availableScientists.map(person => (
                        <AgentRow
                            key={person.id}
                            person={person}
                            action="assign"
                            actionLabel="ASSIGN"
                            onAction={() => assignAgentToResearch(activeRecord.id, person.id)}
                        />
                    ))}
                </div>
            )}
            ...
            <div className="mt-4">
                ...
                {status === 'completed' && (
                    <ActionButton disabled onClick={() => {}} className="w-full">
                        COMPLETED
                    </ActionButton>
                )}
                {status === 'locked' && (
