...
beforeEach(() => {
  ...
  vi.mocked(useNavigationStore).mockImplementation((sel: any) => sel(navState));
  vi.mocked(useGameState).mockReturnValue(mockReadyGameState as GameState);
});

describe('App', () => {
  it('renders LoginScreen when status is idle', () => {
    const storeState = { status: 'idle' as const, newGame: vi.fn(), advanceTo: vi.fn(), pauseAdvance: vi.fn() };
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    vi.mocked(useGameStore).mockImplementation((sel: any) => sel(storeState));
    render(<App />);
  ...
  it('renders AppShell with Empire screen when status is ready', () => {
    const storeState = { status: 'ready' as const, newGame: vi.fn(), advanceTo: vi.fn(), pauseAdvance: vi.fn() };
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    vi.mocked(useGameStore).mockImplementation((sel: any) => sel(storeState));
    render(<App />);
