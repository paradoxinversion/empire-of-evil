import type { GameState, Zone, Person, EffectInstance, Squad } from '../types/index.js';
...
export const getAgentSquad = (state: GameState, agentId: string): Squad | undefined =>
  Object.values(state.squads).find(s => s.memberIds.includes(agentId));

export const getSquadMembers = (state: GameState, squadId: string): Person[] => {
...
export const getDailyBuildingIncome = (state: GameState, config: Config): number => {
  const defs = new Map(config.buildings.map(b => [b.id, b]));
  return Object.values(state.buildings)
    .reduce((sum, b) => sum + (defs.get(b.typeId)?.resourceOutput?.money ?? 0), 0);
};

export const getBuildingIncomeByZone = (state: GameState, config: Config): Record<string, number> => {
  const defs = new Map(config.buildings.map(b => [b.id, b]));
  const result: Record<string, number> = {};
  for (const b of Object.values(state.buildings)) {
    const income = defs.get(b.typeId)?.resourceOutput?.money ?? 0;
    result[b.zoneId] = (result[b.zoneId] ?? 0) + income;
  }
  return result;
};

export const getDailyBuildingUpkeep = (state: GameState, config: Config): number => {
  const defs = new Map(config.buildings.map(b => [b.id, b]));
  return Object.values(state.buildings)
    .reduce((sum, b) => sum + (defs.get(b.typeId)?.upkeepPerDay ?? 0), 0);
};

export const getBuildingUpkeepByZone = (state: GameState, config: Config): Record<string, number> => {
  const defs = new Map(config.buildings.map(b => [b.id, b]));
  const result: Record<string, number> = {};
  for (const b of Object.values(state.buildings)) {
    const upkeep = defs.get(b.typeId)?.upkeepPerDay ?? 0;
    result[b.zoneId] = (result[b.zoneId] ?? 0) + upkeep;
  }
  return result;
};
