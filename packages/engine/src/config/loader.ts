...
export function loadConfig(configDir: string): Config {
    return {
        tileTypes: TileTypesSchema.parse(
            readJson(configDir, "tileTypes.json"),
        ) as Record<string, TileTypeDefinition>,
        buildings: BuildingsSchema.parse(
            readJson(configDir, "buildings.json"),
        ) as BuildingDefinition[],
        pets: PetsSchema.parse(
            readJson(configDir, "pets.json"),
        ) as PetDefinition[],
        worldgen: WorldGenConfigSchema.parse(
            readJson(configDir, "worldgen.json"),
        ),
        activities: ActivitiesSchema.parse(
            readJson(configDir, "activities.json"),
        ) as unknown[],
        citizenActions: CitizenActionsSchema.parse(
            readJson(configDir, "citizenActions.json"),
        ) as unknown[],
        ...
        evilTiers: EvilTiersSchema.parse(
            readJson(configDir, "evilTiers.json"),
        ) as unknown[],
        personAttributes: PersonAttributesSchema.parse(
            readJson(configDir, "personAttributes.json"),
        ) as AttributeDefinition[],
        plots: PlotsSchema.parse(
            readJson(configDir, "plots.json"),
        ) as unknown[],
        researchProjects: ResearchProjectsSchema.parse(
            readJson(configDir, "researchProjects.json"),
        ) as ResearchProjectDefinition[],
        skills: SkillsSchema.parse(
            readJson(configDir, "skills.json"),
        ) as SkillDefinition[],
    };
}
