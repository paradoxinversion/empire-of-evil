...
export function placeBuildings(
  ...
  for (const zone of zones) {
    ...
    const buildingCount = Math.floor(
      (zone.generationWealth / 100) * maxBuildingsPerZone * (zoneSize / avgZoneSize),
    );
    ...
    const eligible = buildings.filter(b => {
      ...
      if ((b.wealthRequirement ?? 0) > zone.generationWealth) return false;
      if (b.buildableOnTileTypes && b.buildableOnTileTypes.length > 0) {
        if (!b.buildableOnTileTypes.includes(zone.pluralityTypeId)) return false;
      }
      return true;
    });
...
function weightedDraw(options: BuildingDefinition[], prng: Prng): BuildingDefinition | undefined {
