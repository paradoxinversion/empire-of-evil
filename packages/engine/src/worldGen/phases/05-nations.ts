...
import { createZone } from "../../index.js";
...
export const resetNationCounter = (v = 0): void => {
    _nationCounter = v;
};
...
export function placeNations(
    ...
    const nationZones = new Map<string, string[]>(
        nationIds.map((id) => [id, []]),
    );
    ...
    while (expanded) {
        ...
        if ([...nationZones.values()].every((c) => c.length >= zonesPerNation))
            break;
    }
    ...
    let empireOriginTile =
        zones.find((z) => z.id === empireOriginZoneId)?.tileIds[0] ??
        "unknown-tile";
    console.warn(
        `Empire origin zone: ${empireOriginZoneId} (tile ${empireOriginTile})`,
    );
    ...
    return { nationZones, empireOriginZoneId, empireOriginTile };
}
