...
function makeZone(
    ...
    return {
        ...
        tileCells: Array.from({ length: tileCount }, (_, i) => ({
            x: i,
            y: 0,
        })),
    };
...
const buildings: BuildingDefinition[] = [
    ...
    },
    {
        ...
        preferredSkills: [],
        wealthRequirement: 30,
        wealthWeight: 3,
        buildableOnTileTypes: ["plains", "mountain"],
    },
    {
        id: "headquarters",
        name: "Headquarters",
...
describe("placeBuildings", () => {
    test("uninhabitable zones get no buildings", () => {
        ...
        const zoneBuildings = Object.values(result.buildings).filter(
            (b) => b.zoneId === "z1",
        );
    ...
    test("headquarters type is never placed by this phase", () => {
        ...
        const hqBuildings = Object.values(result.buildings).filter(
            (b) => b.typeId === "headquarters",
        );
    ...
    test("zone building IDs match buildings whose zoneId matches", () => {
        ...
        const buildingsInZone = Object.values(result.buildings).filter(
            (b) => b.zoneId === "z1",
        );
    ...
    test("placed buildings are assigned a tileId within the zone", () => {
        const zone = makeZone("z1", 4, 80);
        const habitable = new Set(["z1"]);
        ...
        for (const building of zoneBuildings) {
            // `tileId` is a new field we will add; cast to any to keep this test focused on behavior
            expect((building as any).tileId).toBeDefined();
            expect(zone.tileIds).toContain((building as any).tileId);
        }
    });
});
