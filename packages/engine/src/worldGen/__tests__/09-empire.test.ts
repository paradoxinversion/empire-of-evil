...
function makePerson(id: string, zoneId: string, overrides: Partial<Person> = {}): Person {
  return {
    id,
    name: `Person ${id}`,
    zoneId,
...
const empireZoneCitizens = Array.from({ length: 12 }, (_, i) => makePerson(`citizen-${i}`, 'empire-zone'));
const otherZoneCitizens = Array.from({ length: 3 }, (_, i) => makePerson(`other-${i}`, 'other-zone'));
...
describe('initializeEmpire', () => {
  test('overlord person exists in persons result', () => {
    const result = initializeEmpire('empire-zone', 'empire-org', pets, {}, defaultResources, 42, populationPersons);
    expect(result.persons[result.overlordId]).toBeDefined();
  });

  test('overlord has agentStatus with job unassigned', () => {
    const result = initializeEmpire('empire-zone', 'empire-org', pets, {}, defaultResources, 1, populationPersons);
  ...
  test('pet person exists in persons result', () => {
    const result = initializeEmpire('empire-zone', 'empire-org', pets, {}, defaultResources, 5, populationPersons);
    expect(result.persons[result.petId]).toBeDefined();
  });

  test('pet matches petTypeId when provided', () => {
    const result = initializeEmpire('empire-zone', 'empire-org', pets, { petTypeId: 'tiger' }, defaultResources, 3, populationPersons);
  ...
  test('pet is randomly chosen when petTypeId is not provided', () => {
    const result = initializeEmpire('empire-zone', 'empire-org', pets, {}, defaultResources, 7, populationPersons);
    const pet = result.persons[result.petId]!;
    expect(['Cat', 'Tiger']).toContain(pet.name);
  });

  test('headquarters building is placed in the empire zone', () => {
    const result = initializeEmpire('empire-zone', 'empire-org', pets, {}, defaultResources, 2, populationPersons);
  ...
  test('starting resources come from params when provided', () => {
    ...
    const result = initializeEmpire('empire-zone', 'empire-org', pets, { startingResources: customResources }, defaultResources, 1, populationPersons);
  ...
  describe('subject setup', () => {
    test('starting zone citizens have governingOrganizationId set to empire org', () => {
      ...
      for (const citizen of empireZoneCitizens) {
        expect(result.persons[citizen.id]!.governingOrganizationId).toBe('empire-org');
      }
    });

    test('starting zone citizens have initial loyalty of 40 to empire org', () => {
      const result = initializeEmpire('empire-zone', 'empire-org', pets, {}, defaultResources, 1, populationPersons);
      for (const citizen of empireZoneCitizens) {
        expect(result.persons[citizen.id]!.loyalties['empire-org']).toBe(40);
      }
    });
  ...
  describe('agent recruitment', () => {
    test('exactly 10 citizens are recruited as agents when zone has more than 10', () => {
      const result = initializeEmpire('empire-zone', 'empire-org', pets, {}, defaultResources, 1, populationPersons);
      const agents = empireZoneCitizens.filter(c => result.persons[c.id]!.agentStatus !== undefined);
    ...
    test('recruited agents have job unassigned and salary 0', () => {
      ...
      for (const agent of agents) {
        expect(result.persons[agent.id]!.agentStatus!.job).toBe('unassigned');
    ...
    test('all citizens are recruited when zone has fewer than 10', () => {
      const smallPop: Record<string, Person> = Object.fromEntries(
        Array.from({ length: 5 }, (_, i) => makePerson(`small-${i}`, 'empire-zone')).map(p => [p.id, p]),
      );
      const result = initializeEmpire('empire-zone', 'empire-org', pets, {}, defaultResources, 1, smallPop);
      const citizens = Object.values(smallPop);
      const agents = citizens.filter(c => result.persons[c.id]!.agentStatus !== undefined);
      expect(agents).toHaveLength(5);
    });

    test('dead citizens are not recruited as agents', () => {
      const popWithDead: Record<string, Person> = Object.fromEntries([
        ...Array.from({ length: 8 }, (_, i) => makePerson(`live-${i}`, 'empire-zone')),
        makePerson('dead-1', 'empire-zone', { dead: true }),
        makePerson('dead-2', 'empire-zone', { dead: true }),
      ].map(p => [p.id, p]));
      const result = initializeEmpire('empire-zone', 'empire-org', pets, {}, defaultResources, 1, popWithDead);
    ...
    test('recruitment is deterministic for the same seed', () => {
      const r1 = initializeEmpire('empire-zone', 'empire-org', pets, {}, defaultResources, 99, populationPersons);
      const r2 = initializeEmpire('empire-zone', 'empire-org', pets, {}, defaultResources, 99, populationPersons);
      const agents1 = empireZoneCitizens.filter(c => r1.persons[c.id]!.agentStatus !== undefined).map(c => c.id).sort();
      const agents2 = empireZoneCitizens.filter(c => r2.persons[c.id]!.agentStatus !== undefined).map(c => c.id).sort();
